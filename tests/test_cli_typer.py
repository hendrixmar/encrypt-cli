...
def test_app():
    result = runner.invoke(
        app,
        ["input_file.json", "private-key.pem"]
    )
    assert result.exit_code == 0
    assert result.output == 'File control_output.json created!\n'


def test_unexistant_file():
    result = runner.invoke(
        app,
        ["../input_file3.json", "../private-key.pem"]
    )
    assert result.exit_code == 2
    assert ("Error: Invalid value for 'ATTESTATION_FILE': "
            "'../input_file3.json': No such file or directory") in result.output
